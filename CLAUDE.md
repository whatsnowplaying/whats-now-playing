...
- **NEVER** create pull requests against the upstream
  `e1miran/Now-Playing-Serato` repository
...
The application uses a vendored copy of tinytag (`nowplaying/vendor/tinytag/`) for
audio file metadata extraction. The vendor copy includes the M4A multi-value fix
directly (iterates all `data` atoms in `_parse_custom_field`), so no external
monkey patch is needed.

Key API notes for `nowplaying/metadata/tinytag_runner.py`:

- **Core metadata extraction**: `TinyTagRunner` class handles audio file processing
- **Extra fields**: Use `tag.other` (not `tag.extra`)
- **Image processing**: Use `images.as_dict()`
- **Exception handling**: Catch `tinytag.TinyTagException`

**Multi-Value Field Processing:**

Enhanced logic in `tinytag_runner.py` for handling multi-value metadata fields:
...
- **Format-specific handling**:
  - **M4A**: Native multi-value extraction in vendored tinytag
  - **FLAC**: Natural multi-value support from tinytag
  - **MP3**: Delimited string detection and splitting
- **Helper methods**: Use `_process_list_field()` and `_process_single_field()`
