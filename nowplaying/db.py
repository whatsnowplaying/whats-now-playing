...
class MetadataDB:
    ...
    # NOTE: artistfanartraw is never actualy stored in this DB
    # but putting it here triggers side-effects to force it to be
    # treated as binary
    METADATABLOBLIST = [
        'artistfanartraw', 'artistlogoraw', 'artistthumbraw', 'coverimageraw'
    ]
    ...
    def write_to_metadb(self, metadata=None):
        ...
        for (num, key) in enumerate(mdcopy):
            logging.debug("%s %s", num, key)
