...
class GuessGame(GuessGameServerMixin):  # pylint: disable=too-many-instance-attributes
    ...
    def _calculate_points(self, guess: str, guess_type: str, is_first_solver: bool = False) -> int:
        ...
        if guess_type == "letter":
            ...
            if letter_lower in nowplaying.guessgame.scoring.RARE_LETTERS:
                return self._get_config("points_rare_letter", 3, int)
            if letter_lower in nowplaying.guessgame.scoring.COMMON_LETTERS:
    ...
    def _evaluate_letter_guess(  # pylint: disable=too-many-arguments,too-many-positional-arguments
        self,
        guess_text: str,
        track: str,
        artist: str,
        ...
        """
        Evaluate a single-letter guess and update result in place.

        Returns True if guess was already guessed (caller should return early).
        """
    ...
    def _evaluate_word_guess(  # pylint: disable=too-many-arguments,too-many-positional-arguments,too-many-branches,too-many-locals,too-many-statements
        self,
        guess_text: str,
        track: str,
        artist: str,
        ...
        """
        Evaluate a word/phrase guess and update result in place.

        Returns (track_solved, artist_solved) which may have been updated.
        """
        track_lower = track.lower()
        artist_lower = artist.lower()
        guess_normalized = nowplaying.guessgame.scoring.normalize_for_matching(guess_text)
        track_normalized = nowplaying.guessgame.scoring.normalize_for_matching(track_lower)
        artist_normalized = nowplaying.guessgame.scoring.normalize_for_matching(artist_lower)
        logging.debug(
        ...
        if solve_mode == "either":
            if track_match or artist_match:
                ...
                revealed_letters.update(char.lower() for char in track + artist if char.isalpha())
                result["points"] = self._calculate_points(guess_text, "solve", is_first_solver)
                result["solved"] = True
            ...
            ) and nowplaying.guessgame.scoring.phrase_in_guess(artist_normalized, guess_normalized)
            if has_both:
                result["correct"] = True
                result["guess_type"] = "solve"
                revealed_letters.update(char.lower() for char in track + artist if char.isalpha())
            ...
            and nowplaying.guessgame.scoring.phrase_in_guess(track_normalized, guess_normalized)
            and nowplaying.guessgame.scoring.phrase_in_guess(artist_normalized, guess_normalized)
        ):
            result["correct"] = True
            result["guess_type"] = "solve"
            ...
            if artist_solved:
                ...
                result["solve_type"] = "both"
                if is_first_solver:
                    result["points"] += self._get_config("points_first_solver", 50, int)
        ...
        return track_solved, artist_solved

    async def _persist_guess(  # pylint: disable=too-many-arguments,too-many-positional-arguments
        self,
        ...
        """Write guess result to database.

        Updates guesses, user_scores, current_game, and game_history tables.
        """
    ...
    async def process_guess(self, username: str, guess_text: str) -> dict | None:  # pylint: disable=too-many-statements
        """
        ...
        async def _do_process_guess():  # pylint: disable=too-many-locals,too-many-statements
            async with aiosqlite.connect(self.databasefile, timeout=30) as connection:
                ...
                if len(guess_text) == 1 and guess_text.isalpha():
                    already = self._evaluate_letter_guess(
                        guess_text, track, artist, revealed_letters, result
                    )
                    if already:
                        return result
                else:
                    track_solved, artist_solved = self._evaluate_word_guess(
                        guess_text,
                        track,
                        artist,
                        revealed_letters,
