...
class GuessGameServerMixin:
    """Mixin providing server communication for GuessGame.

    Requires the host class to provide: config, stopevent, _http_session,
    is_enabled(), get_leaderboard(), get_current_state().
    """
    ...
    if TYPE_CHECKING:
        ...
        async def get_leaderboard(  # pylint: disable=missing-function-docstring,no-self-use,unused-argument
            self, leaderboard_type: str = "session", limit: int = 10
        ) -> list[dict] | None: ...

        async def get_current_state(self) -> dict | None:  # pylint: disable=missing-function-docstring,no-self-use
            ...
