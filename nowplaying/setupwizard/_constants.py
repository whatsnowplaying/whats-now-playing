#!/usr/bin/env python3
"""Page ID constants for the setup wizard."""

PAGE_WELCOME = 0
PAGE_INPUT = 1
PAGE_INPUT_CONFIG = 2  # dynamic: replaced per-plugin by _InputSourcePage
# 3 was artist extras, now set up from Settings instead. IDs need not be
# contiguous, so the gap is left rather than renumbering the rest.
PAGE_OUTPUTS = 4
PAGE_CONFIGURE_OUTPUTS = 5
PAGE_FINISH = 6
PAGE_MULTIPC = 7  # "Are you using multiple computers?"
PAGE_MULTIPC_ROLE = 8  # "Which machine is this?" (DJ source / display)
PAGE_REMOTE_OUTPUT = 9  # Remote Output config for DJ/source path
