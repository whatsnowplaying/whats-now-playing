...
class DatabaseReader:
    ...
    def get_recent_track_sync(  # pylint: disable=too-many-locals
        self,
    ) -> RekordboxTrack | None:
        ...
        def _query() -> RekordboxTrack | None:
            with sqlite.connect(str(self.database_path)) as conn:  # pylint: disable=no-member
                ...
                # Query for the most recent track from HISTORY ONLY
                # This avoids false positives from tracks loaded on multiple decks
                query = """
                    SELECT
                        h.ID as HistoryID,
                        c.Title,
                ...
                track = RekordboxTrack(
                    identifier=str(history_id) if history_id else "",
                    title=title,
                    artist=artist_name,
                    album=album_name,
                    genre=genre_name,
    ...
    def _get_random_track_from_playlist_sync(  # pylint: disable=too-many-locals
