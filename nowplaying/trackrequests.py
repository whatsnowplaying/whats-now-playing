...
from PySide6.QtWidgets import QComboBox, QHeaderView, QTableWidgetItem, QWidget  # pylint: disable=import-error, no-name-in-module
from PySide6.QtUiTools import QUiLoader  # pylint: disable=import-error, no-name-in-module

import nowplaying.db
import nowplaying.metadata
from nowplaying.exceptions import PluginVerifyError
from nowplaying.subprocesses import SubprocessManager
from nowplaying.uihelp import UIHelp
...
class Requests:  #pylint: disable=too-many-instance-attributes, too-many-public-methods
    ...
    def __init__(self, config=None, stopevent=None, testmode: bool = False, upgrade: bool = False):
        self.config = config
        self.stopevent = stopevent
        self.testmode = testmode
        self.filelists: t.Optional[list[str]] = None
        self.databasefile = pathlib.Path(
            QStandardPaths.standardLocations(QStandardPaths.CacheLocation)[0]).joinpath(
                'requests', 'request.db')
        self.widgets: t.Optional[QWidget] = None
        self.watcher = None
    ...
    async def get_roulette_dupe_list(self, playlist: t.Optional[str] = None) -> list[str]:
        ...
        async with aiosqlite.connect(self.databasefile, timeout=30) as connection:
            ...
            dataset: list[str] = await cursor.fetchall()  # pyright: ignore[reportGeneralTypeIssues]
            if not dataset:
                return []
        return dataset

    @staticmethod
    def normalize(crazystring: str):
    ...
    async def add_to_db(self, data: dict[str, t.Any]):
    ...
    async def add_to_gifwordsdb(self, data: dict[str, t.Any]):
        ''' add an entry to the db '''
        if not self.databasefile.exists():
            logging.error('%s does not exist, refusing to add.', self.databasefile)
            return
    ...
    async def _find_good_request(self, setting: dict[str, str]):
    ...
    async def _get_and_del_request_lookup(self, sql, datatuple) -> dict[str, bytes | str]:
        ''' run sql for request '''
        if not self.databasefile.exists():
    ...
    async def _request_lookup_by_artist_title(self, artist='', title='') -> dict[str, bytes | str]:
    ...
    async def get_request(self, metadata: dict[str, t.Any]) -> dict[str, t.Any]:
        ...
        if not newdata and metadata.get('artist') and metadata.get('title'):
            newdata = await self._request_lookup_by_artist_title(artist=metadata['artist'],
        ...
        if not newdata and metadata.get('title'):
            newdata = await self._request_lookup_by_artist_title(title=metadata['title'])
    ...
    async def check_for_gifwords(self) -> dict[str, bytes | str]:
        ''' check if a gifword has been requested '''
        content: dict[str, bytes | str] = {'reqeuster': '', 'image': '', 'keywords': ''}
        try:
            async with aiosqlite.connect(self.databasefile, timeout=30) as connection:
                connection.row_factory = sqlite3.Row
                cursor = await connection.cursor()
    ...
    async def find_command(self, command: str) -> dict[str, str]:
    ...
    async def find_twitchtext(self, twitchtext: str) -> dict[str, bytes | str | None]:
        ''' locate request information based upon twitchtext '''
        setting: dict[str, str | bytes | None] = {}
        if not twitchtext:
            return setting
    ...
    async def user_track_request(self, setting: dict[str, bytes | str | None], user: str,
                                 user_input: str) -> dict[str, bytes | str | None]:
        ''' generic request '''
        ...
        await self.add_to_db(data)
        newdata: dict[str, bytes | str | None] = {
            'requester': user,
    ...
    async def _tenor_request(self, search_terms: str) -> dict[str, bytes | str | None]:
    ...
    async def gifwords_request(self, setting: dict[str, bytes | str | None], user: str,
                               user_input: str) -> dict[str, bytes | str | None]:
        ''' gifword request '''
    ...
    async def twofer_request(self, setting: dict[str, bytes | str | None], user: str,
                             user_input: str) -> dict[str, bytes | str | None]:
        ''' twofer request '''
    ...
    async def get_all_generator(self) -> t.AsyncGenerator:
    ...
    def get_dataset(self) -> list[dict[str, bytes | str]]:
...
class RequestSettings:
    ...
    def connect(self, uihelp: UIHelp, widget: QWidget):  # pylint: disable=unused-argument
        '''  connect buttons '''
        self.widget = widget
    ...
    def _row_load(self, widget, **kwargs):
        ...
        def _typebox(current: t.Optional[str], enablegifwords=False) -> QComboBox:
            if not current:
                current = 'Generic'
            box = QComboBox()
    ...
    def load(self, config, widget: QWidget):
    ...
    @staticmethod
    def save(config, widget: QWidget, subprocesses: SubprocessManager):  #pylint: disable=unused-argument
        ''' update the twitch settings '''

        def reset_commands(widget: QWidget, config):
