...
class APIResponseCache:
    ...
    def _make_cache_key(self, provider: str, artist_name: str, endpoint: str, 
                ...
                async with aiosqlite.connect(self.db_file) as db:
                    ...
                    if row:
                        ...
                        try:
                            ...
                            logging.debug("Cache HIT for %s:%s:%s (expires in %ds)", 
                                        provider, artist_name, endpoint, expires_at - current_time)
                            return cached_data
    ...
    async def put(self, provider: str, artist_name: str, endpoint: str, 
            ...
            try:
                async with aiosqlite.connect(self.db_file) as db:
                    await db.execute(
                        ...
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                        (cache_key, provider.lower(), artist_name, endpoint, 
                         response_json, current_time, expires_at, current_time)
                    )
                    await db.commit()
                    logging.debug("Cached %s:%s:%s (TTL: %ds)", 
                                provider, artist_name, endpoint, ttl_seconds)
                    
            except sqlite3.Error as e:
                logging.error("Database error storing cache: %s", e)

    @asynccontextmanager
    async def get_or_fetch(self, provider: str, artist_name: str, endpoint: str,
        ...
        try:
            ...
            logging.error("Error fetching data for %s:%s:%s - %s", provider, artist_name, endpoint, e)
            yield None
    ...
    async def get_cache_stats(self) -> dict:
        ...
        try:
            async with aiosqlite.connect(self.db_file) as db:
                ...
                cursor = await db.execute(
                    "SELECT COUNT(*) FROM api_responses WHERE expires_at > ?",
                    (current_time,)
                )
                ...
                by_provider = dict(await cursor.fetchall())
    ...
    async def clear_cache(self, provider: t.Optional[str] = None):
        ...
        async with self._lock:
            try:
                async with aiosqlite.connect(self.db_file) as db:
                    if provider:
                        await db.execute("DELETE FROM api_responses WHERE provider = ?", (provider.lower(),))
