...
async def get_user_image(
    oauth: "nowplaying.twitch.oauth2.TwitchOAuth2", loginname: str
) -> bytes | None:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                f"{oauth.api_host}/users?login={loginname}",
                headers=headers,
                ...
                if resp.status == 200:
                    ...
                    if users := user_data.get("data", []):
                        if profile_image_url := users[0].get("profile_image_url"):
                            ...
                            async with session.get(
                                profile_image_url, timeout=aiohttp.ClientTimeout(total=10)
                            ) as img_resp:
                                if img_resp.status == 200:
                                    raw_image = await img_resp.read()
                                    image = nowplaying.utils.image2png(raw_image)
                                    logging.debug(
                                        "Downloaded profile image for user: %s", loginname
                                    )
                                else:
                                    logging.warning(
                                        "Failed to download profile image: %s", img_resp.status
                                    )
                        else:
