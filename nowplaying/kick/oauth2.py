...
class KickOAuth2:
    ...
    async def exchange_code_for_token(self,
        ...
        if received_state and received_state != self.state:
...
async def main() -> None:
    ...
    if oauth.open_browser_for_auth():
        print(
            f"Please authorize the application and check the redirect URI for the authorization code."
        )
        print(f"The redirect URI should be: {oauth.redirect_uri}")
