...
class KickSettings:
    ...
    def update_oauth_status(self) -> None:
        ...
        status = str(cparser.value(OAUTH_STATUS_KEY, defaultValue=""))
        ...
        if status == OAUTH_STATUS_AUTHENTICATED:
            ...
            self.widget.authenticate_button.setText("Connecting...")
            self.widget.authenticate_button.setEnabled(False)
        else:
    ...
    def update_kickbot_commands(self, config: nowplaying.config.ConfigFile) -> None:
        ...
        for template_path in kickbot_templates:
            ...
            if not existing_keys:
                ...
                config.cparser.endGroup()
                logging.info(
                    "Auto-created Kick command: !%s -> %s", command_name, template_path.name
                )
