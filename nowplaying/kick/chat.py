...
class KickChat:  # pylint: disable=too-many-instance-attributes
    ...
    async def _send_single_message(self, message: str) -> bool:
        ...
        try:
            ...
            safe_headers = {
                k: ("Bearer ***" if k == "Authorization" and v.startswith("Bearer ") else v)
            ...
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                async with session.post(url, headers=headers, json=data) as response:
                    ...
                    logging.debug(
                        "Send message API response: status=%s, body=%s",
                        response.status,
                        response_text,
                    )
    ...
    def _is_duplicate_announcement(self, metadata: dict) -> bool:
        ...
        return self.last_announced["artist"] == metadata.get("artist") and self.last_announced[
            "title"
        ] == metadata.get("title")
