...
class KickLaunch:  # pylint: disable=too-many-instance-attributes
    ...
    def _validate_kick_token_sync(self, access_token: str) -> bool:
        ''' validate kick token synchronously (same as settings) '''
        if not access_token:
            return False
        ...
        try:
            ...
            logging.debug('Token validation response: status=%s', req.status_code)
        ...
        if req.status_code == 200:
            try:
                ...
                if data.get('active'):
                    ...
                    logging.info('Kick token valid - client: %s, type: %s, scopes: %s', client_id,
                                 token_type, scopes)
                    return True
                else:
                    logging.warning('Kick token is inactive')
                    return False
            except Exception as error:  # pylint: disable=broad-except
                logging.error('Kick token validation/bad json: %s', error)
                return False
        elif req.status_code == 401:
            logging.warning('Kick token is invalid/expired')
            return False
        elif req.status_code == 403:
            logging.error('Kick token lacks required permissions - check OAuth scopes')
            return False
        else:
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            if self._validate_kick_token_sync(access_token):
            ...
            if refresh_token:
                ...
                try:
                    ...
                    if new_access_token and self._validate_kick_token_sync(new_access_token):
