...
class MetadataProcessors:  # pylint: disable=too-few-public-methods
    ...
    async def getmoremetadata(self,
                              ...
                              imagecache=None,
                              skipplugins=False) -> dict[str, t.Any]:
        ...
        try:
            for processor in 'hostmeta', 'audio_metadata', 'tinytag', 'image2png':
                ...
                func: t.Callable = getattr(self, f'_process_{processor}')
                func()
    ...
    def _uniqlists(self):
        ...
        if self.metadata.get('artistwebsites'):
            newlist = [
                str(url_normalize.url_normalize(url)) for url in self.metadata['artistwebsites']
            ]
        ...
        if self.metadata.get('artistwebsites'):
            ...
            for url in self.metadata['artistwebsites']:
                ...
                if testurl not in self.metadata['artistwebsites']:
                    newlist.append(url)
            self.metadata['artistwebsites'] = newlist
    ...
    def _musicbrainz(self) -> t.Optional[dict[str, t.Any]]:
...
def recognition_replacement(config=None,
                            metadata: t.Optional[dict[str, t.Any]] = None,
                            addmeta: t.Optional[dict[str, t.Any]] = None) -> dict[str, t.Any]:
