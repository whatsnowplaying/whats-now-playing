...
class DataStorage:
    ...
    async def retrieve_by_url(self, url: str) -> tuple[Any, dict] | None:
        ...
        try:
            ...
            data_value, metadata_json, _ = tuple(result)
...
def run_datacache_maintenance(cache_dir: Path | None = None) -> dict[str, int]:
    ...
    try:
        ...
        def _do_maintenance() -> tuple[int, int]:
            with nowplaying.utils.sqlite.sqlite_connection(str(database_path)) as conn:
                ...
                return expired, requests_cleaned
        ...
        expired_count, requests_count = nowplaying.utils.sqlite.retry_sqlite_operation(
            _do_maintenance
        )
