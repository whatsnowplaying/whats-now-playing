...
def clean_metadata_for_json(obj):
    ...
    if hasattr(obj, "__dict__"):
        return {
            k: clean_metadata_for_json(v) for k, v in obj.__dict__.items() if not k.startswith("_")
        }
    ...
    try:
        ...
        if hasattr(metadata_obj, "pictures") and metadata_obj.pictures:
            metadata["pictures_info"] = {
                "count": len(metadata_obj.pictures),
                "types": [pic.type for pic in metadata_obj.pictures]
                if metadata_obj.pictures
                else [],
                "sizes": [len(pic.data) if pic.data else 0 for pic in metadata_obj.pictures]
                if metadata_obj.pictures
                else [],
            }
...
def compare_metadata(tinytag_data: dict, audio_metadata_data: dict) -> dict[str, Any]:
    ...
    for field in tinytag_fields:
        if (
            field not in mapped_audio_fields
            and field_mappings.get(field, field) not in audio_metadata_fields
        ):
...
def _print_format_analysis(results: list[dict]) -> None:
    ...
    for fmt, stats in formats.items():
        print(
            f"  {fmt.upper()}: TinyTag {stats['tinytag']}/{stats['total']}, "
