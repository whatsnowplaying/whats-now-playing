...
def test_get2(bootstrap):
    ''' test config.get w/TypeErrors '''
    config = bootstrap
    ...
    config.notif = True
    config.cparser.remove('settings/initialized')
    config.cparser.remove('settings/loglevel')
    config.cparser.remove('settings/notif')
    config.cparser.sync()
