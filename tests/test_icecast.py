...
@pytest.mark.parametrize("query_string,expected_artist,expected_title,test_description", [
    # Song field parsing tests
    ('song=Artist%20-%20Title', 'Artist', 'Title', 'song with proper " - " separator'),
    ('song=Artist%20-%20Song%20-%20Remix', 'Artist', 'Song - Remix',
    ...
    ('song=Caf%C3%A9%20-%20R%C3%A9mix', 'Café', 'Rémix',  # codespell:ignore
     'song with URL-encoded special characters'),

    # Separate field tests
    ('artist=Test%20Artist&title=Test%20Title', 'Test Artist', 'Test Title',
...
@pytest.mark.parametrize("test_data,should_log_warning,expected_warning_msg", [
    # Unicode decode errors
    (b'GET /admin/metadata?mode=updinfo&song=\xff\xfe HTTP/1.1', True,
...
@pytest.mark.parametrize("test_data,test_description", [
...
@pytest.mark.asyncio
@pytest.mark.parametrize("metadata_input,expected_output", [
    (None, {}),  # No metadata
    ({'artist': 'Test Artist', 'title': 'Test Title'},
     {'artist': 'Test Artist', 'title': 'Test Title'}),  # With metadata
])
...
async def test_start_port_failure(icecast_bootstrap):
    ...
    with patch('asyncio.get_running_loop') as mock_get_loop:
        ...
        with patch('logging.error') as mock_error:
            ...
            mock_error.assert_called_with('Failed to launch icecast: %s',
                                          mock_loop.create_server.side_effect)
...
@pytest.mark.parametrize("malformed_input,input_description", [
    (b'', 'empty data'),
