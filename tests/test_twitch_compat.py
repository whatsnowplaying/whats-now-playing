...
def test_install_legacy_types(clean_modules):
...
def test_qt_config_serialization_compatibility(test_settings):
...
def test_multiple_install_calls_safe(clean_modules):
...
def test_invalid_refresh_token_exception(clean_modules):
