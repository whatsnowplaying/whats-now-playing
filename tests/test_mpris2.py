...
def test_desc_settingsui_with_dbus():
    ...
    with patch("nowplaying.inputs.mpris2.DBUS_STATUS", True):
        ...
        expected_text = (
            "This plugin provides support for MPRIS2 "
...
async def test_getplayingtrack_with_vlc_metadata(getroot):
    ...
    with patch("nowplaying.inputs.mpris2.DBUS_STATUS", True):
        ...
        test_file_path = os.path.join(
            getroot, "tests", "audio", "15_Ghosts_II_64kb_füllytâgged.m4a"
        )
        ...
        vlc_response = {
            "Metadata": Variant(
                "a{sv}",
                {
                    "mpris:trackid": Variant("o", "/org/videolan/vlc/playlist/15"),
                    "xesam:url": Variant("s", f"file://{test_file_path}"),
                    "xesam:title": Variant("s", "15 Ghosts II"),
                    "xesam:artist": Variant("as", ["Nine Inch Nails"]),
                    "xesam:album": Variant("s", "Ghosts I-IV"),
                    "xesam:tracknumber": Variant("s", "15"),
                    "vlc:time": Variant("u", 113),
                    "mpris:length": Variant("x", 113000000),  # 113 seconds in microseconds
                    "xesam:contentCreated": Variant("s", "2008"),
                    ...
                    "vlc:encodedby": Variant("s", "Lavf61.1.100"),
                    "vlc:length": Variant("x", 113000),
                    "vlc:publisher": Variant("i", 1),
                },
            ),
            "Position": Variant("x", 45000000),  # 45 seconds into the track
