...
async def test_join_phrases_multiartist(getacoustidmbplugin):  # pylint: disable=redefined-outer-name
    ...
    fingerprint_file = pathlib.Path(__file__).parent / 'audio' / '1_Giant_Leap_My_Culture_fingerprint.json'
    with open(fingerprint_file, 'r', encoding='utf-8') as f:
        mock_fingerprint = json.load(f)
    
    # Mock AcoustID lookup response for "My Culture" by 1 Giant Leap feat. Robbie Williams & Maxi Jazz
    mock_acoustid_response = {
        'results': [{
            ...
            'recordings': [{
                ...
                'releases': [{
                    'artists': [{'id': '3eff5a3a-b011-4da3-81fe-bc8d4a11b28c', 'name': '1 Giant Leap'}],
                    'country': 'XE',
                    'date': {'year': 2001},
                    ...
                    'mediums': [{
                        ...
                        'tracks': [{
                            'artists': [
                                {'id': '3eff5a3a-b011-4da3-81fe-bc8d4a11b28c', 'joinphrase': ' feat. ', 'name': '1 Giant Leap'},
                                {'id': 'db4624cf-0e44-481e-a9dc-2142b833ec2f', 'joinphrase': ' & ', 'name': 'Robbie Williams'},
                                {'id': 'debd408d-72b3-4c14-a0eb-dd4fe526e240', 'name': 'Maxi Jazz'}
                            ],
    ...
    with unittest.mock.patch('nowplaying.recognition.acoustidmb.Plugin._fpcalc', return_value=mock_fingerprint), \
    ...
    # Note: The final result may show "and" instead of "&" due to MusicBrainz processing
    assert metadata['artist'] in [
        '1 Giant Leap feat. Robbie Williams & Maxi Jazz',     # Direct from AcoustID
        '1 Giant Leap feat. Robbie Williams and Maxi Jazz'    # After MusicBrainz processing
    ]
