...
def test_imagecache(bootstrap):
    ...
    with tempfile.TemporaryDirectory() as newpath:
        ...
        cachedir = pathlib.Path(newpath).joinpath('cachdir')
        mycache = nowplaying.imagecache.ArtistFanartCache(
            cachedir=cachedir, initialize=True, databasefile=databasefile)
        mycache.start_pool()
        ...
        for diskcache in list(mycache.cache.iterkeys()):
            ...
            data1 = mycache.database.find_artist_cachekey(name, cachekey)
        ...
        # need to do this otherwise Windows may not close
